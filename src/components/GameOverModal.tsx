...
const GameOverModal: React.FC<GameOverModalProps> = ({ players, winner, onRestart }) => {
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
      <Card className="w-full max-w-md p-6 bg-white rounded-lg shadow-2xl animate-fade-in">
        ...
        {winner ? (
          ...
          <div className="text-center mb-6">
            <h3 className="text-2xl font-semibold">It's a Tie!</h3>
            <p className="mt-2 text-lg">
        ...
        <div className="flex justify-between mb-6">
          {players.map(player => (
            <div key={player.id} className="text-center">
              ...
              {player.master && (
                <p className={`text-sm text-${player.master.color}`}>
                  {player.master.name}
                </p>
              )}
            </div>
        ...
        <div className="flex justify-center">
          <Button 
            onClick={onRestart}
            className="px-8 py-2 bg-peggle-blue hover:bg-blue-600 text-white font-semibold rounded-lg shadow-md"
          >
            Play Again
          </Button>
        </div>
